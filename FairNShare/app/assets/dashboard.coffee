   ...
   $(document).on('click', '#task-page', ( ->
      $.get "/showTasks",(tasks) ->
         ...
         $.each tasks,(index,task) ->
            $('#task_div'). append $('<tr><td><li class="list-group-item"></td>').text task.title+" ---- "+task.emailAssignedTo+" ---- "+task.createdBy+" ---- "+task.startDate+" ---- "+task.endDate+" ---- "+task.oldPoints.toFixed(2) 
            ))
